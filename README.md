...
배너를 유지해야 한다면 다른 접근이 필요한데, 아래 두 가지는 **아직 시도하지 않았고
효과도 검증하지 않았음.** 개념상 가능하다는 수준의 정리임.

| 방법 | 상태 | 근거 | 대가 |
|---|---|---|---|
| 배너 제거 | **적용함** | **실측 −5089 µs** (5091 → 2 µs) | 부팅 확인 수단 상실 |
| 보드레이트 상향 (115200 → 921600) | 미시도 | 전송 시간이 보드레이트에 반비례한다는 **계산상 추정**. 실제로는 UART가 그 속도를 지원하는지, 전송 외 오버헤드가 얼마나 되는지 확인해야 함 | 호스트 설정 변경 필요 |
| 비동기 UART (인터럽트/DMA) | 미시도 | CPU가 전송 완료를 기다리지 않게 되므로 대기 시간이 사라진다는 **개념 수준의 판단** | 구현 복잡도 증가 |
