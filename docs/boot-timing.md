...
- 출력: UART 콘솔 (115200 8N1 — 7장의 3차 실험은 921600)
...
배너를 유지해야 한다면 — **보드레이트를 115200 → 921600으로 올리면 `APPLICATION` 5091 → 635 µs (1/8), total 780 µs** (7장).
이 시간이 CPU 연산이 아니라 UART 전송 시간이라는 것을 두 번째 실험으로 확인함.
...
## 7. 3차 실험 — 배너를 유지하고 보드레이트를 올리면

6장에서 "배너를 지우는 대신 보드레이트를 올리는 방법은 시도하지 않았음"이라고 남겨 두었음.
그 방법을 실측함. **가설을 먼저 적고 측정했음.**

### 가설 (측정 전 기록)

```
115200 bps → 문자당 86.8 µs
921600 bps → 문자당 10.85 µs  (8배)
배너 약 59자 → APPLICATION 5091 µs 가 약 640 µs 부근으로 줄 것으로 예상

미확인 ① 호스트 쪽 가상 COM 포트(ST-LINK VCP)가 921600을 받는가
미확인 ② 전송 외 고정 오버헤드가 얼마나 되는가 — 정확히 1/8이 안 되면 그 차이
```

### 방법 — Zephyr 소스 무수정

콘솔 속도는 Kconfig가 아니라 **Device Tree**에 있음 (`&usart2 { current-speed = <115200>; }`).
보드 `.dts` 원본은 그대로 두고, 애플리케이션 쪽 overlay 파일 한 개로 덮어씀.
...
`boards/<board>.overlay` 는 빌드 시 자동으로 합쳐짐. `prj.conf` 에서는 `CONFIG_BOOT_BANNER=n` 을 제거해 배너를 복구함.

적용 확인은 빌드 산출물의 최종 Device Tree에서:

```bash
$ sed -n '/usart2:/,/};/p' build/zephyr/zephyr.dts
        usart2: serial@40004400 {
                ...
                status = "okay";                     /* in zephyr/boards/st/nucleo_f446re/nucleo_f446re.dts:99 */
                current-speed = < 0xe1000 >;         /* in ../app/boards/nucleo_f446re.overlay:2 */
        };
```

`0xe1000` = 921600. 오른쪽 주석이 **값의 출처 파일:행**이라, 다른 속성은 SoC/보드 dts에서 오고
이 한 줄만 overlay 에서 왔음이 그대로 보임.

호스트 터미널도 같은 속도로: `python -m serial.tools.miniterm /dev/tty.usbmodem* 921600`
...
- **5091 ÷ 635 = 8.02** — 보드레이트를 8배 올리자 정확히 1/8이 됨. 가설(약 640 µs)과 5 µs 차이.
- 따라서 `APPLICATION` 구간은 CPU가 일하는 시간이 아니라 **UART가 문자를 밀어내는 시간**이고, 보드레이트에 정확히 반비례함.
  6장의 "병목은 연산이 아니라 동기 I/O 대기"라는 결론이 두 번째 실험으로 확인됨.
- 미확인 ① → ST-LINK VCP는 921600을 깨짐 없이 받았음.
- 미확인 ② → 전송 외 오버헤드는 측정 한계 내에서 0. 배너 시간은 순수 전송 시간임.
- 다른 네 구간(465 / 10620 / 2217 / 579 cyc)은 그대로 — 변인이 하나였음.
- **921600에서도 배너가 전체의 81%** (635 / 780). 8배 빨라져도 여전히 지배적임.

### 두 실험의 순서가 다름

| 실험 | 순서 |
| ---- | ---- |
| 5장 배너 제거 (−5089 µs) | **먼저 재보고 나서 알았음.** 측정 전 예상은 `PRE_KERNEL_1`이었고 틀렸음 |
| 7장 보드레이트 상향 (635 µs) | **측정 전에 가설을 적어두고** 재서 맞았음 |

가설이 먼저 기록돼 있어야 "예상했고 맞았다"라고 쓸 수 있음. 그렇지 않으면 결과에 맞춰 예측을 쓰는 것이 됨.

### 선택의 문제로

| 방법 | APPLICATION | 남는 것 |
| ---- | ----------: | ------- |
| 배너 제거 | 2 µs | 부팅 확인 수단 없음 |
| 배너 유지 + 921600 | 635 µs | 부팅 확인 수단 유지, 호스트 설정 변경 필요 |

부팅 확인 수단을 남기는 비용이 약 0.6 ms 임. 어느 쪽을 택할지는 제품 요구에 달린 문제이고,
이 실험은 그 비용을 숫자로 만든 것임. 비동기 UART(인터럽트/DMA)는 시도하지 않았음.

---

## 다음

배너 제거 후 남은 146 µs 의 구성 (배너 유지 + 921600 기준으로는 780 µs 중 145 µs).
